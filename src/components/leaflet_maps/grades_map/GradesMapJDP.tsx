...
const GradesMap: React.FC = () => {
  ...
  const mapStyle: StyleFunction<any> = (feature) => {
    ...
    const score =
      scores.find((o) => o.woj === feature?.properties.nazwa)?.JDP ?? 0; //If 'score' is 'undefined', use 0 as the default value
  ...
  const onEachFeature = (feature: GeoJSON.Feature, layer: Layer) => {
    const score = scores.find((o) => o.woj === feature.properties?.nazwa)?.JDP;
    const scoreToString =
      score == null || score == undefined ? "brak" : score.toString();
    if (feature.properties && feature.properties.nazwa) {
      layer.bindPopup(
        // Bind a popup with the region's name and score
        `<strong>Województwo: ${feature.properties.nazwa}<br>Ocena: ${scoreToString}</strong>`,
        { autoPan: false }
      );
    }
  };
  ...
  const createScoreMarkers = (geojsonData: GeoJSON.FeatureCollection) => {
    return geojsonData.features.map((feature: GeoJSON.Feature) => {
      const score =
        scores.find((o) => o.woj === feature.properties?.nazwa)?.JDP ?? 0;
      if (
